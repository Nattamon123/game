...
class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("bg", "./image/marr.avif");
    ...
    shootMountain(mountainId) {
        ...
        if (gameState.mountains.length === 0) {
            ...
            if (!hasForLoop) {
                logToConsole('⚠️ คุณต้องใช้ for loop เพื่อทำภารกิจให้สำเร็จ!');
                logToConsole('💡 ตัวอย่าง: for (let i = 1; i <= 6; i++) { console.log(i); }');
    ...
    showVictoryDialog(hasForLoop) {
        ...
        const messages = hasForLoop ? [
            { text: "ยินดีด้วย! คุณทำภารกิจสำเร็จแล้ว! 🎉", delay: 0 },
            { text: "คุณใช้ for loop ได้อย่างถูกต้อง! 🏆", delay: 0 },
            { text: "คุณเป็นโปรแกรมเมอร์ที่เก่งที่สุด! 💻", delay: 0 }
        ] : [
            { text: "ยินดีด้วย! คุณทำภารกิจสำเร็จแล้ว! 🎉", delay: 0 },
            { text: "ลองใช้ for loop เพื่อทำให้โค้ดสั้นลง! 💡", delay: 0 },
            { text: "คุณสามารถทำได้ดีกว่านี้! 🚀", delay: 0 }
        ];
    ...
    runTestCase() {
        ...
        // ตรวจสอบการใช้ for loop
        const hasForLoop = code.includes('for(') || code.includes('for (');
        if (!hasForLoop) {
            logToConsole('⚠️ โปรดใช้ for loop เพื่อยิงภูเขาทั้งหมด');
            logToConsole('💡 ตัวอย่าง: for (let i = 1; i <= 6; i++) { console.log(i); }');
            return;
        }
...
require(["vs/editor/editor.main"], function () {
        ...
        value: `/**
 ใช้คำสั่ง for loop เพื่อยิงภูเขาทั้งหมด
 สามารถใช้คำสั่ง console.log(หมายเลขภูเขา) เพื่อยิงภูเขาที่ต้องการ แต่ต้องใช้คำสั่งนี้ใน 
 for loop เท่านั้น
 **/
for (let i = /เริ่มต้นที่เลขไหน?/ ; i <= /หยุดเมื่อไหร่?/ ; i++) {
    // เขียนโค้ดของคุณที่นี่
    console.log(/ตัวแปรที่ใช้ใน for loop/);
}
