...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        const messages = [
            { text: "ยินดีต้อนรับสู่ด่านที่ 3 ของภารกิจนี้", delay: 0 },
            { text: "ใช้คำสั่ง while loop เพื่อยิงภูเขาทั้งหมด", delay: 0 },
    ...
    shootMountain(mountainId) {
        ...
        if (gameState.mountains.length === 0) {
            ...
            if (!hasWhileLoop) {
                ...
                logToConsole('💡 ตัวอย่าง: let i = 1; while (i <= 6) { console.log(i); i++; }');
            ...
            this.showVictoryDialog(hasWhileLoop);
    ...
    showVictoryDialog(hasWhileLoop) {
        const dialogBox = this.add.graphics();
        const x = 90;
        const y = 290;
        ...
        const messages = [
            ...
            { text: hasWhileLoop ? 
                "เยี่ยมมาก! คุณใช้ while loop ได้ถูกต้อง!" : 
                "ลองใช้ while loop ในครั้งต่อไปนะ!", 
                delay: 0 },
    ...
    runTestCase() {
        ...
        if (!hasWhileLoop) {
            logToConsole('⚠️ โปรดใช้ while loop เพื่อยิงภูเขาทั้งหมด');
            logToConsole('💡 ตัวอย่าง: let i = 1; while (i <= 6) { console.log(i); i++; }');
            return;
        }
    ...
    analyzeCodeComplexity(code) {
        ...
        const hasForLoop = /\bfor\s*\([^)]*\)\s*\{[^}]*\}/.test(code);
        
        const hasIfStatement = /\bif\s*\([^)]*\)\s*\{[^}]*\}/.test(code);
...
require(["vs/editor/editor.main"], function () {
        ...
        value: `/**
 ใช้คำสั่ง while loop เพื่อยิงภูเขาทั้งหมด
